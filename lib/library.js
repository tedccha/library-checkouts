// Using cloakbrowser instead of Playwright because the library catalog
// is protected by Cloudflare bot detection which blocks headless browsers.
// cloakbrowser provides better stealth capabilities to bypass this.
...
async function getBrowser() {
  if (!browser) {
    try {
      ...
      throw new Error("Browser automation unavailable. Library catalog fetch requires headless browser support.");
    }
  }
  return browser;
}
...
async function loginAndGetPage() {
  ...
  await page.fill('input#username', process.env.LIBRARY_USERNAME);
  await page.fill('input#password', process.env.LIBRARY_PASSWORD);
    ...
    try {
      await page.waitForFunction(
        () => {
          ...
          const hasChallenge = html.includes("Just a moment") ||
                              html.includes("Performing security verification") ||
                              html.includes("Cloudflare");
          // Check if we have actual content
          const hasContent = document.querySelectorAll('div.result').length > 0 ||
                            document.querySelectorAll('table tr').length > 1;
          return !hasChallenge && hasContent;
        },
        { timeout: 45000 }
      );
      console.log("✓ Cloudflare challenge completed, books loaded");
    } catch {
      console.log("⚠ Cloudflare challenge or page load timeout, checking for content anyway...");
    }
...
export async function fetchHolds(username, password) {
  ...
  try {
    ...
    try {
      await page.waitForFunction(
        () => document.querySelectorAll('div.result').length > 0,
        { timeout: 10000 }
      );
      console.log("✓ Holds loaded");
    } catch {
      console.log("⚠ Holds page loading");
    }
